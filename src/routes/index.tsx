...
import { Sparkles, Terminal, Copy, History, ThumbsUp, AlertTriangle, X } from 'lucide-react'
...
function NeoButton({ onClick, children, className = "", variant = "primary" }: { onClick?: () => void, children: React.ReactNode, className?: string, variant?: "primary" | "secondary" | "danger" | "success" }) {
  const bgColors = {
    primary: "bg-pink-400",
    secondary: "bg-white",
    danger: "bg-red-400",
...
function TypewriterText({ text, delay = 30, startDelay = 0, onComplete }: { text: string, delay?: number, startDelay?: number, onComplete?: () => void }) {
    ...
    <div className="min-h-screen bg-yellow-50 font-sans text-black flex flex-col items-center justify-center p-6 relative overflow-x-hidden">
      ...
      <div className="max-w-xl w-full relative z-10">
          ...
          {currentHaiku ? (
            <div key={currentHaiku.id} className="text-center space-y-6 w-full">
              ...
              <div className="text-2xl md:text-3xl font-bold italic leading-tight bg-yellow-200 inline-block px-2 py-1 border border-black rotate-[-1deg] shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                <TypewriterText text={`"${currentHaiku.line1}"`} startDelay={0} />
              </div>
              <br />
              <div className="text-2xl md:text-3xl font-bold italic leading-tight bg-green-200 inline-block px-2 py-1 border border-black rotate-[1deg] shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                <TypewriterText text={`"${currentHaiku.line2}"`} startDelay={1000} />
              </div>
              <br />
              <div className="text-2xl md:text-3xl font-bold italic leading-tight bg-cyan-200 inline-block px-2 py-1 border border-black rotate-[-1deg] shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                <TypewriterText text={`"${currentHaiku.line3}"`} startDelay={2000} />
              </div>
            </div>
          ) : (
        ...
        {currentHaiku && (
          <div className="flex gap-4 justify-center mb-6">
            <NeoButton 
              ...
              className="flex-1"
            >
              <ThumbsUp className={`w-5 h-5 ${rating === 'like' ? 'fill-black' : ''}`} />
              [ COMPILE ]
            </NeoButton>
            <NeoButton 
              ...
              className="flex-1"
            >
              <AlertTriangle className={`w-5 h-5 ${rating === 'dislike' ? 'fill-black' : ''}`} />
              [ ERROR ]
            </NeoButton>
          </div>
        )}
        ...
        <div className="flex justify-center gap-4">
          ...
          <NeoButton onClick={() => setShowLogs(true)} variant="secondary" className="px-4">
      ...
      >
        <div className="h-full flex flex-col p-6 overflow-y-auto bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px]">
          <div className="flex justify-between items-center mb-6 pb-4 border-b-4 border-black border-dashed">
            ...
            <button onClick={() => setShowLogs(false)} className="p-2 hover:bg-red-100 rounded-full border-2 border-transparent hover:border-black transition-all">
              <X className="w-8 h-8" />
            </button>
          </div>
          
          {history.length === 0 ? (
            <div className="text-gray-400 font-mono text-center mt-10">NO_LOGS_FOUND</div>
          ) : (
            <div className="space-y-6 font-mono text-sm">
               {history.map((item, index) => (
                 <div key={`${item.id}-${index}`} className="border-2 border-black p-4 bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
